...
import nanogui
# import vae.config
from vae.datapipeline import build_constraint_kdtree
# import vae.utils
...
from viewer.datasources import (PointCloud, TriangleMesh,
                                VectorCloud)
...
class ViewerState:
    def __init__(self, scatterviewer, inPos=None, coeffs=None, need_projection=False, ws_coeffs=False, poly_order=3, prediction_space='TS'):
    ...
    def get_reconstructed_mesh(self,level=0.0):
        res = 128
        v = self.viewer
        if self.rec_mesh is None and self.coeffs is not None:
            ...
            self.rec_mesh = get_coeff_trianglemesh(self.inPos, v.inDirection, v.face_normal, self.coeffs,
                                                   ...
                                                   v.min_pos, v.max_pos, res, self.prediction_space,level=level)
...
def setup_mesh_for_viewer_2(mesh_file, sigma_t, g, albedo):
    ...
    # breakpoint()
    constraint_kd_tree, sampled_p, sampled_n = build_constraint_kdtree(mesh_file, sigma_t, g, albedo)
...
def get_coeff_trianglemesh(in_pos, in_dir, normal, poly_coeffs, poly_order, scale_factor,
                           min_pos, max_pos, res, prediction_space,level=0.0):
    ...
    # breakpoint()
    # verts, faces, _, _ = measure.marching_cubes(f_taylor, 0.50)
    #  breakpoint()
    verts, faces, _, _ = measure.marching_cubes(f_taylor, level)
    # verts, faces, _, _ = measure.marching_cubes(f_taylor, 0.0)
