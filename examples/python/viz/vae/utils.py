...
def polynomial_to_voxel_grid_new(pos, in_dir, normal, poly_coeffs, poly_order,
    ...
    # Evaluate taylor expansion
    dim = pos.shape[1]
    poly_coeffs = poly_coeffs.squeeze()
    coords = []
    for d in range(dim):
        coords.append(np.linspace(extent[2 * d], extent[2 * d + 1], res) + 0.5 / res)
    components = np.meshgrid(*coords, indexing='ij')
    ...
    # breakpoint()
    if prediction_space == "WS":
        coords_transformed = utils.transforms.transform_to(coords, pos, in_dir, normal, prediction_space)
    else:
        coords_transformed, _ = utils.transforms.transform_to(coords, pos, in_dir, normal, prediction_space)
    # breakpoint()
