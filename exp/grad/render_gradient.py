...
def opt_task(isSweep=True):
    ...
    def compute_FD(A,S,sensor_id,delta=1.0,poly_fit=False,chunk=False,idx_param=2):
        # Render image
        # seed = 2
        ...
        if (delta == 0.0).sum() > 0:
            grad = (img_pos[...,idx_param] - img_neg[...,idx_param]) / 2.0 / delta[idx_param].cpu().numpy()
            breakpoint()
        else:
            grad = (img_pos - img_neg) / (2*delta.cpu().numpy()) 
            grad = grad.mean(axis=-1) # Make single channel
    ...
    if args.render_results:
        ...
        if isAlbedo:
            ...
            exp_name = f"sensor{sensor_id}_sigmaT{sc.param_map[material_key].sigma_t.data[idx_param][0]}_{sc.param_map[material_key].type_name()}_delta{fd_delta}"
        ...
        result_fd = compute_FD(A,S,sensor_id,delta=param_delta,idx_param=idx_param,
                               poly_fit=False,chunk=False)
        ...
        # Save Abs. Diff.
        result_absdiff = np.abs(result_fd - result_diff)
        fname = fname.replace('FD','absdiff')
        np.save(fname,result_absdiff)
    else:
        optTask()
